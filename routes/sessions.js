    ...
    },
    {
      $addFields: {
        chestStatusCounts: {
          $reduce: {
            input: "$chests",
            initialValue: { CREATED: 0, UPLOADED: 0, PROCESSING: 0, PROCESSED: 0, ERROR: 0 },
            in: {
              CREATED: {
                $cond: [{ $eq: ["$$this.status", "CREATED"] }, { $add: ["$$value.CREATED", 1] }, "$$value.CREATED"]
              },
              UPLOADED: {
                $cond: [{ $eq: ["$$this.status", "UPLOADED"] }, { $add: ["$$value.UPLOADED", 1] }, "$$value.UPLOADED"]
              },
              PROCESSING: {
                $cond: [{ $eq: ["$$this.status", "PROCESSING"] }, { $add: ["$$value.PROCESSING", 1] }, "$$value.PROCESSING"]
              },
              PROCESSED: {
                $cond: [{ $eq: ["$$this.status", "PROCESSED"] }, { $add: ["$$value.PROCESSED", 1] }, "$$value.PROCESSED"]
              },
              ERROR: {
                $cond: [{ $eq: ["$$this.status", "ERROR"] }, { $add: ["$$value.ERROR", 1] }, "$$value.ERROR"]
              }
            }
          }
        }
